...
class UserFundsTransferController extends Controller
{
    ...
    public function check(Request $request, $reference){
        ...
        if ($transfer->verify == $request->code){
            ...
            if ($transfer->type == 1){
                ...
                session()->flash('message', 'After Charging Transfer Fee Your Total Sending Amount NGN' .$settings->currency .$net_amount.' Has Been Successfully Send.');
                Session::flash('type', 'success');
                ...
                $receiver->notify(new RecivedMoneySuccess($data));
                session()->flash('message', 'After Charging Transfer Fee Your Total Sending Amount '.$settings->currency .$net_amount.' Has Been Successfully Send.');
                Session::flash('type', 'success');
                Session::flash('title', 'Send Successful');
                return redirect()->route('userFundsTransfer');
    ...
    public function others(Request $request){
        ...
        if ($receiver_user == null){
            session()->flash('message', "There is no user that has '".$request->email."' email address. Please check your email first.");
            Session::flash('type', 'error');
        ...
        elseif ($type == 2){
            ...
            if ($settings->minimum_transfer > $reamount){
                ...
                session()->flash('message', "The Minimum Amount to Transfer ".$settings->currency .$settings->minimum_transfer." Please Increase money first.");
                Session::flash('type', 'error');
                Session::flash('title', 'Minimum Withdraw');
                return redirect()->route('userFundsTransfer');
            }
