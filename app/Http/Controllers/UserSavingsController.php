...
class UserSavingsController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('user.savings.index', compact('rewards'));
    }
       ...
       public function history()
    {
        ...
        $logs = Saving::whereUser_id($user->id)->orderBy('updated_at', 'desc')->paginate(15);
        
        $time = date('M j, Y  H:i:s', strtotime($user->bonus));
        $rewards = json_encode($time);

        return view('user.savings.history', compact( 'logs','rewards'));
    }
     ...
     public function log(Request $request, $id){
        ...
        $time = date('M j, Y  H:i:s', strtotime($user->bonus));
        $rewards = json_encode($time);
        $logs = Savings_payment::wheretransaction_id($id)->paginate(20);
        $total = Savings_payment::wheretransaction_id($id)->select('amount')->sum('amount');;
      
       
        return view('user.savings.log',compact('total', 'logs','rewards'));
    }
    ...
    public function cryptoConfirm(Request $request)
    {
     ...
     public function submit(Request $request){
        ...
        $settings = Settings::first();
         ...
         if ($request->amount > $user->profile->deposit_balance ){

            session()->flash('message', "You cant create a ".$request->name." savings plana s your current balance of ".$settings->currency." ".$user->profile->deposit_balance." Is not enough. Please deposit money first or try transfer your all money to deposit balance using fund transfer.");
            Session::flash('type', 'error');
            Session::flash('title', 'Insufficient Funds');

            return redirect()->route('userSavings');
            ...
            return view('user.savings.preview',compact('invest','rewards'));
        ...
        $settings = Settings::first();
         if ($request->amount > $user->profile->deposit_balance ){
            ...
            session()->flash('message', "You cant save now on your ".$request->name." savings wallet as your current balance of ".$settings->currency." ".$user->profile->deposit_balance." Is not enough. You need ".$settings->currency." ".$request->amount." to proceed. Please deposit money first or try transfer your all money to deposit balance using fund transfer.");
            Session::flash('type', 'error');
            Session::flash('title', 'Insufficient Funds');
            ...
            return view('user.savings.preview2',compact('count','invest','rewards'));
        ...
        $user->profile->savings_balance = $user->profile->savings_balance + $request->amount;

        $user->profile->save();

	    $pay = new Saving ();
        $pay->user_id = $user->id;
        $pay->transaction_id = $request->code;
        $pay->amount = $request->amount;
        $pay->plan = $request->name;
        $pay->cycle = $request->cycle;
        $pay->next = $request->date;
        
        $pay->save();

        $pay2 = new Savings_payment ();
        $pay2->user_id = $user->id;
        $pay2->transaction_id = $request->code;
        $pay2->payment_id = str_random(12);
        $pay2->amount = $request->amount;
        $pay2->plan = $request->name;
        $pay2->cycle = $request->cycle;
        
        $pay2->save();

        
        session()->flash('message', 'You Have Successfully Started A '.$request->name.' Savings Scheme.');
        Session::flash('type', 'success');
        Session::flash('title', 'Invest Successful');

        return redirect()->route('userSavings.history');
    }
     ...
     public function confirm2(Request $request){
        ...
        DB::table('savings')->where('transaction_id', $request->code)->update(['next' => $request->date]);
        ...
        session()->flash('message', 'You Have Successfully Deposited  '.$settings->currency.''.$request->amount.' into your '.$request->name.' Savings Scheme. with reference number '.$request->code.'');
        Session::flash('type', 'success');
        Session::flash('title', 'Invest Successful');
